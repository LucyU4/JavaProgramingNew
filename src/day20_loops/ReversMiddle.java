...
for (int i = middleWord.length()-1; i>=0; i--){
    reverseMiddle += middleWord.charAt(i);// loop starts from the last index of the middle word snd goes until the first index in a backward order/
    reverseMiddle += middleWord.charAt(i);// reading each based on the int i value, which is going backwards and then concatenating that character to the reverseMiddle String
}
        System.out.println(str.substring(0, str.indexOf(' ')) + reverseMiddle + " "+str.substring((end+1)));
//str.substring(o,str,indexOf(' ') gives the first word
//str.substring(0,str,indexOf(' ') could have been written as: str.subString(0, start -1)
        ...
        // str.replace(middleWord,reverseWord) another option.
    }
}
/*
take a look at, alternative o reverse middle
String str. + I love java"
String reverse = "";
